...
impl SortConfig {
    ...
    pub fn validate(&self) -> SortResult<()> {
        ...
        if let Some(buffer_size) = self.buffer_size {
            if buffer_size < 1024 {
                return Err(SortError::invalid_buffer_size("buffer size too small (minimum 1KB)"));
            }
            if buffer_size > 1024 * 1024 * 1024 * 8 {  // 8GB limit
                return Err(SortError::invalid_buffer_size("buffer size too large (maximum 8GB)"));
            }
        }
        ...
        if let Some(threads) = self.parallel_threads {
            if threads == 0 {
                return Err(SortError::thread_pool_error("thread count must be positive"));
            }
            if threads > 1024 {
                return Err(SortError::thread_pool_error("too many threads (maximum 1024)"));
            }
        }
    ...
    pub fn reading_from_stdin(&self) -> bool {
        ...
        (self.input_files.len() == 1 && self.input_files[0] == "-")
    }
...
mod tests {
    ...
    fn test_sort_mode_from_str() {
        assert_eq!("numeric".parse::<SortMode>().expect("Failed to parse numeric mode"), SortMode::Numeric);
        assert_eq!("version".parse::<SortMode>().expect("Failed to parse version mode"), SortMode::Version);
